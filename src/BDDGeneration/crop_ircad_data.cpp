...
int main(int argc,char** argv)
{
    ...
    // Ircad is annoying so we have to check also that values are OK.....
    itk::ImageRegionIterator<MaskImageType> itM( imgMask,imgMask->GetLargestPossibleRegion() );
    itM.GoToBegin();

    itk::ImageRegionIterator<VesselsImageType> itV( imgVessels,imgPortal->GetLargestPossibleRegion() );
    itV.GoToBegin();

    itk::ImageRegionIterator<VesselsImageType> itP( imgPortal,imgPortal->GetLargestPossibleRegion() );
    itP.GoToBegin();
    while( !itM.IsAtEnd() )
    {
        if(itM.Get() == 1)
