...
void itk::OptimallyOrientedFlux<TInputImage,TOutputImage>::GenerateData() 
{
   ...
   using FFTType = itk::FFTWForwardFFTImageFilter<TInputImage>;
    ...
    for(int i=0; i<size[0];i++)
        for(int j=0; j<size[1];j++)
            for(int k=0; k<size[2]; k++)
            {
                ...
                index[1] = j;
                index[2] = k;
                value = sqrtf32( vector[0]->GetPixel(index) * vector[0]->GetPixel(index) +
                                    vector[1]->GetPixel(index) * vector[1]->GetPixel(index) +
                                    vector[2]->GetPixel(index) * vector[2]->GetPixel(index)   )
                                    + 1e-12;
    ...
    {
        normalization = 4.0/3.0*M_PI*(rad*rad*rad)/boost::math::cyl_bessel_j<float,float>(1.5,2*M_PI*rad*1e-12)/std::pow(1e-12,1.5)
        / (rad*rad) * ( rad / std::pow( sqrtf32( 2*rad*m_sigma - m_sigma*m_sigma ),m_normalizationType ) );
