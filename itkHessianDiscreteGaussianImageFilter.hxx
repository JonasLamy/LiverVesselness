 ...
 *
 *=========================================================================*/
#ifndef itkHessianDiscreteGaussianImageFilter_hxx
#define itkHessianDiscreteGaussianImageFilter_hxx
...
::HessianDiscreteGaussianImageFilter()
{
  ...
  typename GaussianFilterType::OrderArrayType firstOrder;
  firstOrder.Fill(1);
  ...
  m_DerivativeFilterA->SetOrder(firstOrder);
  m_DerivativeFilterA->SetNormalizeAcrossScale(m_NormalizeAcrossScale);

  m_DerivativeFilterB->SetOrder(firstOrder);
...
::SetSigma(RealType sigma)
{
  ...
  for ( unsigned int i = 0; i < numberOfSmoothingFilters; i++ )
    {
    m_SmoothingFilters[i]->SetVariance(sigma);
    }
  m_DerivativeFilterA->SetVariance(sigma);
  m_DerivativeFilterB->SetVariance(sigma);
 ...
 */
template< typename TInputImage, typename TOutputImage >
typename HessianDiscreteGaussianImageFilter< TInputImage, TOutputImage >
::RealType
 HessianDiscreteGaussianImageFilter< TInputImage, TOutputImage >
::GetSigma() const
{
...
::GenerateInputRequestedRegion()
{
  ...
  typename HessianDiscreteGaussianImageFilter< TInputImage,
...
::GenerateData()
{
  ...
  typename GaussianFilterType::OrderArrayType zeroOrder;
  zeroOrder.Fill(0);
  typename GaussianFilterType::OrderArrayType firstOrder;
  firstOrder.Fill(1);
  typename GaussianFilterType::OrderArrayType secondOrder;
  secondOrder.Fill(2);
  ...
  for ( unsigned int dima = 0; dima < ImageDimension; dima++ )
    ...
    for ( unsigned int dimb = dima; dimb < ImageDimension; dimb++ )
      ...
      if ( dimb == dima )
        {
        m_DerivativeFilterA->SetOrder(secondOrder);
        m_DerivativeFilterB->SetOrder(zeroOrder);
